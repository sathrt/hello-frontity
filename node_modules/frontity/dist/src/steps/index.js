...
const createTsConfig = async (path) => {
    const fileTemplate = await fs_extra_1.readFile(path_1.resolve(__dirname, "../../templates/tsconfig.json"), {
        encoding: "utf8",
    });
    const filePath = path_1.resolve(path, "tsconfig.json");
    await fs_extra_1.writeFile(filePath, fileTemplate);
};
...
const initializeGit = async (path) => {
    try {
        child_process_1.execSync("git init", { cwd: path, stdio: "ignore" });
        child_process_1.execSync("git add .", { cwd: path, stdio: "ignore" });
        child_process_1.execSync('git commit -m "Initialized with Frontity"', {
            cwd: path,
            stdio: "ignore",
        });
    }
        ...
        await fs_extra_1.remove(path_1.resolve(path, ".git"));
...
const createGitignore = async (path) => {
    const fileTemplate = await fs_extra_1.readFile(path_1.resolve(__dirname, "../../templates/gitignore-template"), {
        encoding: "utf8",
    });
    const gitignorePath = path_1.resolve(path, ".gitignore");
    const gitignoreExists = await fs_extra_1.pathExists(gitignorePath);
    if (gitignoreExists) {
        // Keep the existing .gitignore in memory in case we need to revert to it later.
        const backupGitignore = await fs_extra_1.readFile(gitignorePath, {
            encoding: "utf8",
        });
        // Append if there's already a `.gitignore` file
        fs_extra_1.appendFileSync(gitignorePath, "node_modules\nbuild");
        // Return a "cleanup" function
        return async () => {
            await fs_extra_1.writeFile(gitignorePath, backupGitignore);
        };
    }
    else {
        await fs_extra_1.writeFile(gitignorePath, fileTemplate);
        // Return a "cleanup" function
        return async () => {
            // Remove the .gitignore file
            await fs_extra_1.remove(gitignorePath);
        };
    }
};
...
const subscribe = async (email) => {
    ...
    return node_fetch_1.default("https://n8n.frontity.org/webhook/62923334-59a4-484c-a9c2-632814b94225", {
        method: "POST",
        body: JSON.stringify({
